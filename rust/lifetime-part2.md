...
объявления параметров времени жизни очень похожи на
[шаблоны](https://doc.rust-lang.org/stable/book/generics.html). В этом
объявлении после ключевого слова `impl` задаём имена для параметров времён
жизни. Их может быть несколько. В нашем примере используется один параметр
времени жизни `a`.
...
объектов происходит в обратном порядке их объявлений. Таким образом на не
продолжительное время `bob` будет хранить в себе ссылку на уже освобождённый
...
Этот код не компилируется, т.к. время жизни ссылки `fabia` меньше чем у `bob`. С
точки зрения разработчика код безопасен, т.к. в конце внутренней области
видимости `bob` больше не ссылается на `fabia`. Компилятор строго следует нашему
объявлению метода `buy_car(&mut self, c: &'a Car)`, где мы указали что экземпляр
структуры `Car` имеет время жизни `a` и экземпляр `Customer` не может иметь
время жизни больше чем у экземпляра `Car`.
...
Можно иметь **множество заимствования неизменяемых ссылок** на экземпляр, если
нет заимствования изменяемой ссылки.
...
fn main() {
    ...
    bob.buy_car(&fabia);    // bob заимствует неизменяему ссылку на fabia 

    let p1 = &fabia;        // Можем ещё заимствовать неизменяемые ссылки
    let p2 = &fabia;
}
```
