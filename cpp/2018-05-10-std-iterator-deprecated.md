...
Скорее всего в вашем проекте не используется C++17 на данный момент. Но рано или 
поздно вам придётся его использовать. Лучше уже сегодня подготовиться и 
...
Что должен вывести этот код? В описании для функции `std::reduce` говорится, что 
она вернёт объект, который будет инициализирован значением по умолчанию, 
используя пустые фигурные скобки `{}`. В нашем случае это будет `int{}`, который 
имеет значение `0`.
...
Если вы хотите получить доступ к этим пяти типам у итератора, то можно подумать, 
что все итераторы определяют все эти пять типов. Например, можно получить тип 
элемента: `Iterator::value_type`.
...
В случае с указателем мы не можем получить тип элемента `int*::value_type`, т.к. 
указатель не имеет вложенных типов!

Чтобы решить эту проблему, предлагается **не вызывать напрямую** `::value_type` 
или `::iterator_category`, а использовать шаблон `std::iterator_traits`, который 
предоставляет  все пять типов.
...
предлагает ещё причину для прекращения использования `std::iterator`. Отсутствие 
ясности кода ещё заметнее при определении итератора вывода:
...
Для того, чтобы убедить комитет стандартизации C++ отказаться от `std::iterator` 
достаточно было причины отсутствия ясности кода, но есть ещё одна причина 
данного подхода при наследовании: вы не можете обращаться напрямую к псевдонимам 
типов базового класса. Например, вы не можете получить тип `value_type`:
...
Но этот код не выполняет требования STL по определению пяти типов для итератора, 
но может работать на некоторых платформах. Такой код не переносим на другие 
платформы. Правильно будет объявить все пять типов итератора:
...
Итак, почему некоторые платформы заставляют вас писать все 5 типов, даже если вы 
их не используете?
